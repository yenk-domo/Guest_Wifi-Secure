...
$button_click =
{
	...
	if ((!$compagnyname) -or (!$numbercomputer) -or (!$mail))
	{
		...
		$returnStatus.BackColor = "Transparent"
		$returnStatus.ForeColor = "Red"
		$returnStatus.Text = "Complet all the fields! If the problem persists contact myCompagny."
	...
	else
	{
		if (!(Test-Path -Path "C:\Program files\SecureWifi\mac1.txt"))
		{
			$returnStatus.BackColor = "Transparent"
			$returnStatus.ForeColor = "Red"
			$returnStatus.Text = "Click on check ! If the problem persists contact myCompagny."
		}
		else
		{
			...
			$file = new-item "C:\Program Files\SecureWifi\myCompagny.txt" -type file -value "$Value" -Force;
			
			#Upload the file to the FTPServer
			$client = New-Object System.Net.WebClient;
			$client.UploadFile("ftp://UserFTP:P4$$W0rd@<externalIP>/WifiSecure/myCompagny.txt", "C:\Program Files\SecureWifi\myCompagny.txt");
